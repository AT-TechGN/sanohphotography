...
const PhotosManagement = () => {
  ...
  const handleFileUpload = async (files) => {
    if (!selectedAlbum) {
      showError('Sélectionnez d\'abord un album');
      return;
    }
    ...
    try {
      ...
      await photoService.uploadPhotos(selectedAlbum.id, fileArray);
  ...
  const handleToggleFeatured = async (photoId, isFeatured) => {
    try {
      ...
      setPhotos(photos.map(p => 
        p.id === photoId ? { ...p, isFeatured: !isFeatured } : p
      ));
    ...
    <div className="p-4 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            Gestion Photos & Albums
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {albums.length} album(s) • {stats?.totalPhotos || 0} photo(s)
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-5 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm">Total Albums</p>
              <p className="text-3xl font-bold">{stats?.totalAlbums || 0}</p>
            </div>
            <FolderIcon className="w-10 h-10 text-white/30" />
          </div>
        </div>
        <div className="bg-gradient-to-br from-pink-500 to-pink-600 rounded-2xl p-5 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-pink-100 text-sm">Total Photos</p>
              <p className="text-3xl font-bold">{stats?.totalPhotos || 0}</p>
            </div>
            <PhotoIcon className="w-10 h-10 text-white/30" />
          </div>
        </div>
        <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-2xl p-5 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-cyan-100 text-sm">Albums Publics</p>
              <p className="text-3xl font-bold">{stats?.publicAlbums || 0}</p>
            </div>
            <EyeIcon className="w-10 h-10 text-white/30" />
          </div>
        </div>
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-5 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold">{stats?.photosThisMonth || 0}</p>
            </div>
            <CloudArrowUpIcon className="w-10 h-10 text-white/30" />
          </div>
        </div>
      </div>
              ...
              >
                ...
                {bookings.map((booking) => (
                  <option key={booking.id} value={booking.id}>
                    {booking.client} - {booking.service} ({booking.date})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 border-l border-gray-200 dark:border-gray-600 pl-4">
                <button
                  ...
                  className={`p-2 rounded-lg ${viewFormat === 'grid' ? 'bg-purple-100 text-purple-600' : 'text-gray-500'}`}
                >
                  <Squares2X2Icon className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setViewFormat('list')}
                  className={`p-2 rounded-lg ${viewFormat === 'list' ? 'bg-purple-100 text-purple-600' : 'text-gray-500'}`}
                >
          ...
          {filteredAlbums.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredAlbums.map((album) => (
                <div
                  key={album.id}
                  className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden group"
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <span>{album.photosCount} photo(s)</span>
                    </div>
                    ...
                    {album.booking && (
                      <div className="flex items-center gap-2 mt-2 text-sm text-purple-600">
                        ...
                        <span>{album.booking.service}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Album</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Catégorie</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Photos</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredAlbums.map((album) => (
                    <tr key={album.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {album.booking && <p className="text-sm text-purple-600">{album.booking.client}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-xs font-medium">{album.category}</span>
                      </td>
                      <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{album.photosCount} photos</td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          album.isPublic ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-400'
                        }`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => loadAlbumPhotos(album.id)} className="p-2 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-lg" title="Voir"><EyeIcon className="w-5 h-5" /></button>
                          <button onClick={() => editAlbum(album)} className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg" title="Modifier"><PencilIcon className="w-5 h-5" /></button>
                          <button onClick={() => handleDeleteAlbum(album.id)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="Supprimer"><TrashIcon className="w-5 h-5" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {viewMode === 'album-detail' && selectedAlbum && (
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-4 mt-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    selectedAlbum.isPublic ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700'
                  }`}>
              ...
              <button onClick={() => editAlbum(selectedAlbum)} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30">
          ...
          <div
            className={`bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border-2 border-dashed transition-colors ${
              dragActive ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-600'
            }`}
          ...
          >
            <div className="text-center">
              <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
                dragActive ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-500'
              }`}>
              ...
              <label
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-medium hover:shadow-lg transition-all cursor-pointer"
              >
                {uploading ? (
                  <>
              ...
              {photos.map((photo) => (
                <div key={photo.id} className="group relative aspect-square bg-gray-100 dark:bg-gray-700 rounded-xl overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleToggleFeatured(photo.id, photo.isFeatured)}
                        className={`p-2 rounded-full transition-colors ${
                          photo.isFeatured ? 'bg-yellow-400 text-white' : 'bg-white/20 text-white hover:bg-white/40'
                        }`}
                    ...
                    <p className="text-white text-xs text-center px-2 truncate">{photo.originalFilename}</p>
                  </div>
                  {photo.isFeatured && (
